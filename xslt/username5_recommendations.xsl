			...
			<body>
				...
				<table border="1">
					<tr>
						...
						<xsl:variable name="username" select="'username5'"></xsl:variable>
						<xsl:variable name="recommendations"
							select="/ReviewApplication/Recommendations/Recommendation[UserName=$username]"></xsl:variable>
						<tr>
							<td>
								<xsl:value-of select="$username"></xsl:value-of>
							</td>
							<td>
								<xsl:value-of select="count($recommendations)"></xsl:value-of>
							</td>
						</tr>
				</table>
			</body>
